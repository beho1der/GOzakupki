Сервис используется для взаимодействия с системой гос закупок(https://zakupki.gov.ru/) для получения данных о государственных закупках по идентификатору закупки. Предусмотренно повторение запросов при неудаче,а также парелельный запрос при вытаскивании всех эапов закупки.
...
----- запрос с скачиванием файлов в S3 -----
*curl -X POST --data '{"id": "2920456270218000088","timeout":20,"repeat":2,"fileDownload":true}'  http://127.0.0.1:8025/api/get*
...
*fileDownload* - true/false, скачивать ли приложенные файлы в S3 хранилище  
...
*file* - массив приложенных файлов (заполняется при fileDownload=true)  
  *url* - исходная ссылка на файл  
  *name* - имя файла  
  *size* - размер файла в байтах  
  *s3Bucket* - имя S3 бакета  
  *s3Key* - ключ файла в S3  
  *s3ETag* - ETag объекта в S3  
...
PROXY_URL "http://127.0.0.1:3128" - IP или URL с портом для доступа к серверу.     
 ...
 ------ S3 ----------
 S3_ENDPOINT "minio.example.com" - Адрес S3 хранилища (MinIO, AWS S3 и т.д.).  
 S3_REGION "us-east-1" - Регион S3 хранилища.  
 S3_ACCESS_KEY "" - Access Key для авторизации в S3.  
 S3_SECRET_KEY "" - Secret Key для авторизации в S3.  
 S3_BUCKET "" - Имя бакета для сохранения файлов.  
 S3_USE_SSL "true" - Использовать SSL для подключения к S3.  